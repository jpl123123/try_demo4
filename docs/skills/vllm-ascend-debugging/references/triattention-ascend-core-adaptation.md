...
9. **信号-事件闭环**：scheduler 发信号 → worker 压缩 → 事件回传 → scheduler 回收 →
   ...
   排查时按 §9 观测层逐环看。
10. **驱逐功能清单参照（本项目定位）**：所有 KV"压缩"本质上都是驱逐，本文件是
    各维度驱逐物理闭环的实现参照（SKILL.md §2.3d「驱逐视角」）——按
    `N_tokens/N_layer/N_head/d_head/dtype` 五个维度对照本文件的
    选择（§2 信号）/搬移（§5 compact 原语）/回收（§6）/同步（§4+§6）/计数修正（§4）
    五步，把每个维度的驱逐尽可能打通到物理回收，而不是只做读视图假驱逐。
